...
const EditJob = () => {
  ...
  // console.log(resume);
  const job = resume.jobs.filter((j) => {
    if (j.id === id) {
      return j;
    }
  });
  const [formData, setFormData] = useState({
    ...
    profile: job[0].profile,
    organization: job[0].organization,
    location: job[0].location,
    startDate: new Date(job[0].startDate),
    endDate: new Date(job[0].endDate),
  ...
  const SubmitForm = () => {
    ...
    dispatch(asyncEditJob(formData));
  };
