...
import { asyncGetInternship } from "../store/actions/internshipActions";
import { asyncApplyInternship } from "../store/actions/studentActions";
...
const DetailInternship = () => {
  ...
  useEffect(() => {
    dispatch(asyncGetInternship(id));
  }, []);
  ...
  const Apply = () => {
    ...
    dispatch(asyncApplyInternship(id)).then((status) => {
      if (status === 200) {
        // <toast
      ...
      {!internship ? (
        ...
        >
          ...
          <span className="text-slate-600 text-fs-1 border-b-2 py-2 sm:text-sm">
            <p>
              <i className="ri-play-circle-line me-2"></i> Starting from:-{" "}
              {internship.from}
            </p>
            <p>
              <i className="ri-calendar-line me-2"></i>Duration:-{" "}
              {internship.duration}
            </p>
          </span>
          ...
          <span className="text-slate-600 text-fs-1 border-b-2 py-2 sm:text-sm">
            ...
            <p className="mt-3 mb-1">Responsibility:- </p>
